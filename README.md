...
[![CI](https://github.com/abdrahmentakrouni/linux-hardening/actions/workflows/ci.yml/badge.svg)](https://github.com/abdrahmentakrouni/linux-hardening/actions/workflows/ci.yml)
...
```bash
git clone https://github.com/abdrahmentakrouni/linux-hardening.git
cd linux-hardening
...
```bash
sudo bash harden.sh --apply --yes --port 2222 --allow-ports "80,443,8443"
sudo bash harden.sh --apply --only ssh,firewall          # just the network basics
sudo bash harden.sh --audit --skip fail2ban,autoupdates  # audit everything else
```
